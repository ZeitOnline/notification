...
describe.sequential('notification spoken accessibility', () => {
	...
	it('exposes top alerts and speaks the action and close buttons in tab order', async () => {
		await virtual.start({ container: document.body });
		await virtual.interact();
	...
	it('speaks link actions when navigating top notifications by keyboard', async () => {
		await virtual.start({ container: document.body });
		await virtual.interact();
